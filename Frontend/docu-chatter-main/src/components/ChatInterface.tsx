...
import { Send, Mic, MicOff, Volume2, VolumeX, ThumbsUp, ThumbsDown } from 'lucide-react';
...
export const ChatInterface = ({ chatbot, chatbotName, onSendMessage }: ChatInterfaceProps) => {
  ...
  const handleUrlSubmit = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Please refresh the page and select the scraped URL file from the 'Select Documents' option.",
        duration: 10000,
      });
      setUrlInput("");
      return result;
  ...
  const handleSave = (overrides: Partial<typeof tempSettings> = {}) => {
    ...
    try {
      localStorage.setItem(`model-settings-${chatbot.id}`, JSON.stringify(settings));
  ...
  const customMultiStyles = {
    control: (base: any) => ({ ...base, minHeight: 40, borderRadius: 8, fontSize: 14, paddingLeft: 2, borderColor: '#d1d5db', boxShadow: 'none', '&:hover': { borderColor: '#9ca3af' } }),
    menu: (base: any) => ({ ...base, zIndex: 50, fontSize: 14 }),
    option: (base: any, { isFocused }: any) => ({ ...base, backgroundColor: isFocused ? '#f3f4f6' : 'white', color: '#111827', cursor: 'pointer' }),
    multiValue: (base: any) => ({ ...base, backgroundColor: '#e5e7eb', borderRadius: 4, padding: '2px 6px' }),
    multiValueLabel: (base: any) => ({ ...base, color: '#111827', fontWeight: 500 }),
    multiValueRemove: (base: any) => ({ ...base, color: '#6b7280', ':hover': { backgroundColor: '#d1d5db', color: '#111827' } }),
  };
                      ...
                      <div className="flex items-center justify-between mt-2">
                        ...
                        <div className="flex gap-2">
                          ...
                          <Popover
                            ...
                            onOpenChange={(isOpen) => setOpenPopoverId(isOpen ? message.id : null)}
                          >
                            ...
                            <PopoverContent className="w-64">
                              <div className="space-y-2">
                                <Label htmlFor={`comment-${message.id}`}>Comment</Label>
                                <Input id={`comment-${message.id}`} placeholder="Enter your feedback" value={comment} onChange={(e) => setComment(e.target.value)} />
                                <Button size="sm" className="w-full" onClick={() => handleCommentSubmit(message.id)}>Submit</Button>
                              </div>
                            </PopoverContent>
                          </Popover>
                          <DropdownMenu>
      ...
      <div>
        ...
        <div className="max-h-[150px] overflow-y-auto px-4 py-2 border-t border-chatbot-primary/20 space-y-2">
          <div className="flex items-center flex-wrap gap-5 px-4 py-2 border-t border-chatbot-primary/20">
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" size="default">Documents</Button>
              </PopoverTrigger>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-1">
                        ...
                        <Select
                          onValueChange={(value) => setTempSettings({ ...tempSettings, llmModel: value })}
                          disabled={isLoading}
                        >
                          <SelectTrigger>
                            <SelectValue placeholder={tempSettings.llmModel !== "" ? tempSettings.llmModel : "Select LLM"} />
                          </SelectTrigger>
                          <SelectContent>
                      ...
                      <div className="space-y-1">
                        ...
                        <Input
                          ...
                          onChange={(e) => setTempSettings({ ...tempSettings, tokenSize: Number(e.target.value) })}
                          disabled={isLoading}
                        />
                      ...
                      <div className="space-y-1">
                        ...
                        <Select
                          onValueChange={(value) => setTempSettings({ ...tempSettings, guardrailOption: value })}
                          disabled={isLoading}
                        >
                          <SelectTrigger>
                            <SelectValue placeholder={tempSettings.guardrailOption !== "" ? tempSettings.guardrailOption : "Select Guardrail Level"} />
                          </SelectTrigger>
                          <SelectContent>
                      ...
                      <div className="space-y-2">
                        ...
                        <Slider
                          ...
                          onValueChange={(val) => setTempSettings({ ...tempSettings, temperature: val[0] })}
                          disabled={isLoading}
                        />
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-1">
                        ...
                        <Select
                          onValueChange={(value) => setTempSettings({ ...tempSettings, optimizer: value })}
                          disabled={isLoading}
                        >
                          <SelectTrigger>
                            <SelectValue placeholder={tempSettings.optimizer !== "" ? tempSettings.optimizer : "Select Optimizer"} />
                          </SelectTrigger>
                          <SelectContent>
                      ...
                      <div className="space-y-1">
                        ...
                        <Select
                          onValueChange={(value) => setTempSettings({ ...tempSettings, embeddingModel: value })}
                          disabled
                        >
                          <SelectTrigger>
                            <SelectValue placeholder={tempSettings.embeddingModel !== "" ? tempSettings.embeddingModel : "all-MiniLM-L6-v2"} />
                          </SelectTrigger>
                          <SelectContent>
                      ...
                      <div className="space-y-1">
                        ...
                        <Select
                          onValueChange={(value) => setTempSettings({ ...tempSettings, vectorDb: value })}
                          disabled
                        >
                          <SelectTrigger>
                            <SelectValue placeholder={tempSettings.vectorDb !== "" ? tempSettings.vectorDb : "FAISS"} />
                          </SelectTrigger>
                          <SelectContent>
                      ...
                      <div className="space-y-1">
                        ...
                        <select
                          ...
                          onChange={(e) => setTempSettings({ ...tempSettings, rerankerOption: e.target.value })}
                          disabled={isLoading}
                        >
                          <option value="none">None – Use retriever results directly</option>
                          <option value="cross-encoder">Cross-encoder – Most accurate, but slower</option>
                          <option value="bi-encoder">Bi-encoder – Faster, less accurate</option>
                          <option value="llm-reranker">LLM-based – Uses a language model</option>
                        </select>
                        <p className="text-xs text-muted-foreground">
                          Choose a re-ranking method to reorder retrieved documents.
                        </p>
                      </div>
                    </div>
                  </div>
