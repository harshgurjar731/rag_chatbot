...
export const useChatbots = () => {
  ...
  const fetchChatbotsWithFiles = async () => {
    try {
      ...
      const res = await axios.get<RawDatastore[]>("http://127.0.0.1:8000/datastore/");
      const datastores = res.data;

      // Step 2: Fetch files for each datastore
      const chatbotPromises = datastores.map(async (ds) => {
        let files: FileRecord[] = [];

        try {
          const fileRes = await axios.get<FileRecord[]>(
            `http://127.0.0.1:8000/datastores/${ds.id}/files`
          );
          files = fileRes.data;
        } catch (fileErr) {
          console.warn(`Failed to fetch files for datastore ${ds.id}`, fileErr);
        }
        ...
        return {
          ...
          updatedAt: ds.updated_at ? new Date(ds.updated_at) : new Date(ds.created_at),
          datastoreId: ds.id,
        };
      });
