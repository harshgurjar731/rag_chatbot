...
def insert_qna(session: Session, datastore_id: int, qna_json: dict, file_id: int = None):
    """
...
# Build the dynamic path (points to Data folder inside project)
    ...
    for file in files:
        # Skip if curated QA JSON already exists
        qa_file_name = f"{file.filename.split('.')[0]}_qa_pairs_cleaned.json"
        qa_file_path = output_dir / qa_file_name

        if qa_file_path.exists():
            print(f"[INFO] QA file already exists for {file.filename}, skipping.")
            continue
        ...
        # Insert QA into DB
        inserted = insert_qna(session, datastore_id=datastore_id, qna_json=qna_json, file_id=file.id)
