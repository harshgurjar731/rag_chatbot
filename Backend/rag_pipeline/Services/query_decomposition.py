...
def format_conversation_history(history: list[tuple[str, str]]) -> str:
    ...
    print(f"Formatted conversation history with {len(history)} entries")
    return formatted
...
def normalize_relevance_scores(
    ...
    if not documents:
        print("No documents provided for normalization")
        return documents

    print(f"Normalizing relevance scores for {len(documents)} documents")
    ...
    for doc in documents:
        if "relevance_score" in doc.metadata:
            ...
            print(f"Normalized score: {original_score} -> {normalized_score}")
...
def merge_contexts(
    ...
    reranker: CrossEncoderReranker | BiEncoderReranker | LLMReranker | None = None,
    filter_docs: bool = True,
) -> list[Document]:
    """
    ...
    if filter_docs and reranker:
        ...
        all_contexts = reranker.rerank(query=query, documents=all_contexts)
        all_contexts = normalize_relevance_scores(all_contexts, filter_docs=False)

    return all_contexts


def generate_subqueries(query: str, llm: ChatOpenAI | AzureChatOpenAI | None) -> list[str]:
    """
    ...
    """
    template = RAG_PROMPTS["query_decomposition_multiquery_prompt"]
    ...
    print(f"Generated {len(questions)} subqueries from original query")
    print(f"Subqueries: {questions}")

    return questions


def rewrite_query_with_context(
    question: str, history: list[tuple[str, str]], llm: ChatOpenAI|AzureChatOpenAI|None
) -> str:
    """
    ...
    """
    if not history:
        print("No history available, returning original question")
        return question

    query_rewriter_prompt = RAG_PROMPTS["query_decompositions_query_rewriter_prompt"]
...
def retrieve_and_rank_documents(
    query: str,
    original_query: str,
    vdb: QDrantVectorDB | None,
    ranker: CrossEncoderReranker | BiEncoderReranker | LLMReranker | None,
    embedding: HuggingFaceEmbeddings|OpenAIEmbeddings|None,
    collection_name: str,
    top_k: int,
    ranker_top_k: int,
    ...
    returned_docs_list = vdb.test_retrieval(
        queryList=[query],
...
def generate_answer_for_query(
    question: str, documents: list[Document], llm: ChatOpenAI|AzureChatOpenAI|None
) -> str:
    """
    ...
    """
    rag_template = RAG_PROMPTS["query_decomposition_rag_template"]
...
def generate_followup_question(
    ...
    followup_question_prompt = ChatPromptTemplate.from_messages(
        [
            ("system", RAG_PROMPTS["query_decomposition_followup_question_prompt"])
        ]
    )
    ...
    followup_question = followup_question_chain.invoke(
        {
            "conversation_history": history, #format_conversation_history(history),
    ...
    if cleaned_followup:
        print(f"Generated follow-up question: {cleaned_followup}")
    else:
        print("No follow-up question needed")
...
def generate_final_response(
    ...
    """
    final_response_prompt = RAG_PROMPTS["query_decomposition_final_response_prompt"]
 ...
 # ✅ Chain: prompt → LLM → output parser
    final_response_chain = (
        {"question": lambda x: x["question"], "context": lambda x: x["context"], "conversation_history": lambda x: x["conversation_history"]}
        | final_response_generator
        | llm
        | StrOutputParser()
    )

    print("Generating final comprehensive response")

    # Raw LLM output
    final_answer = final_response_chain.invoke({"question": original_query, "context": contexts, "conversation_history": history})
...
def iterative_query_decomposition(
    ...
    collection_name: str,
    top_k: int,
    ranker: CrossEncoderReranker | BiEncoderReranker | LLMReranker | None = None,
    recursion_depth: int = 3,
    enable_citations: bool = True,
    ranker_top_k: int = 10,
    confidence_threshold: float = 0.0,
    llm_settings: dict[str, Any] | None = None,
    embedding: HuggingFaceEmbeddings|OpenAIEmbeddings|None = None,
    ...
    """
    print(f"Starting query decomposition for: '{query[:100]}...'")

    if not vdb:
        print("No retriever provided")
    ...
    if len(questions) == 1:
        print("No decomposition needed, using RAG directly")
        single_query = query

        # Retrieve and rank documents for the single query
        retrieved_docs = retrieve_and_rank_documents(
            single_query, query, vdb, ranker, embedding, collection_name, top_k, ranker_top_k, []
        )
    ...
    for depth in range(recursion_depth):
        print(f"Recursion depth: {depth + 1}/{recursion_depth}")
        ...
        if followup_question.strip().strip("'").strip('"'):
            ...
            print(f"Continue with follow-up question: {followup_question}")
        else:
            print(f"No follow-up needed, stopping at depth {depth + 1}")
            break

    # Search document from original query as well
    retrieved_docs = retrieve_and_rank_documents(
        query, query, vdb, ranker, embedding, collection_name, top_k, ranker_top_k, []
    )
    ...
    print("Generating final response with all collected contexts")
    return generate_final_response(
        conversation_history,
        contexts,
        query,
