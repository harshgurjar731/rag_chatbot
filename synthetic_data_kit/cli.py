...
from synthetic_data_kit.utils.config import load_config, get_openai_config, get_azure_openai_config, get_llm_provider, get_path_config
...
def system_check(
    ...
    if selected_provider == "api-endpoint":
        ...
        azure_config = get_azure_openai_config(ctx.config)
        
        api_key = azure_config.get("api_key")
        api_base = azure_config.get("azure_endpoint")
        ...
        with console.status(f"Checking Azure OpenAI access..."):
            try:
                ...
                if not api_key or not api_base or not deployment_name:
                    console.print("L Missing required Azure configurations (api_key, azure_endpoint, or deployment_name)", style="red")
                    return 1
                ...
                console.print(f" Azure OpenAI access confirmed", style="green")
                console.print(f"Endpoint: {api_base}", style="green")
                console.print(f"Deployment: {deployment_name}", style="green")
                console.print(f"Response: {response.choices[0].message.content}", style="green")
                return 0
                
            except Exception as e:
                console.print(f"L Error connecting to Azure OpenAI: {str(e)}", style="red")
                return 1
